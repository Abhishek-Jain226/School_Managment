...
//        }
    ...
    @Override
    public ApiResponse getStudentsForTripAssignment(Integer ownerId) {
        System.out.println("🔍 getStudentsForTripAssignment called with ownerId: " + ownerId);
        try {
            // Validate vehicle owner exists
            VehicleOwner owner = vehicleOwnerRepository.findById(ownerId)
                    .orElseThrow(() -> new ResourceNotFoundException("Vehicle owner not found with ID: " + ownerId));
            ...
            // Get all students from schools where owner's vehicles are assigned
            List<Map<String, Object>> students = new ArrayList<>();
            
            for (Vehicle vehicle : ownerVehicles) {
                // Get school-vehicle mappings for this vehicle
                List<SchoolVehicle> schoolVehicles = schoolVehicleRepository.findByVehicle(vehicle);
                
                for (SchoolVehicle schoolVehicle : schoolVehicles) {
                    School school = schoolVehicle.getSchool();
                    
                    // Get all students from this school
                    List<Student> schoolStudents = studentRepository.findBySchool(school);
                    
                    for (Student student : schoolStudents) {
                        // Check if student is already in our list
                        boolean studentExists = students.stream()
                            .anyMatch(s -> s.get("studentId").equals(student.getStudentId()));
                        ...
                        if (!studentExists) {
                            ...
                            studentMap.put("schoolId", school.getSchoolId());
                            studentMap.put("schoolName", school.getSchoolName());
            ...
            System.out.println("🔍 Found " + students.size() + " students for trip assignment");
            return new ApiResponse(true, "Students for trip assignment retrieved successfully", students);
            
        } catch (Exception e) {
            System.out.println("🔍 Error in getStudentsForTripAssignment: " + e.getMessage());
            e.printStackTrace();
            return new ApiResponse(false, "Error retrieving students: " + e.getMessage(), null);
        }
    }

    @Override
    public ApiResponse getTripsByOwner(Integer ownerId) {
        System.out.println("🔍 getTripsByOwner called with ownerId: " + ownerId);
        try {
            // Validate vehicle owner exists
            VehicleOwner owner = vehicleOwnerRepository.findById(ownerId)
                    .orElseThrow(() -> new ResourceNotFoundException("Vehicle owner not found with ID: " + ownerId));
            ...
            for (Vehicle vehicle : ownerVehicles) {
                List<Trip> vehicleTrips = tripRepository.findByVehicle(vehicle);
                ...
                for (Trip trip : vehicleTrips) {
                    ...
                    tripMap.put("tripType", trip.getTripType());
                    ...
                    tripMap.put("vehicleId", vehicle.getVehicleId());
                    tripMap.put("vehicleNumber", vehicle.getVehicleNumber());
                    tripMap.put("schoolId", trip.getSchool().getSchoolId());
                    tripMap.put("schoolName", trip.getSchool().getSchoolName());
                    tripMap.put("driverId", trip.getDriver() != null ? trip.getDriver().getDriverId() : null);
                    tripMap.put("driverName", trip.getDriver() != null ? trip.getDriver().getDriverName() : null);
            ...
            System.out.println("🔍 Found " + trips.size() + " trips for owner");
            return new ApiResponse(true, "Trips for owner retrieved successfully", trips);
            
        } catch (Exception e) {
            System.out.println("🔍 Error in getTripsByOwner: " + e.getMessage());
            e.printStackTrace();
    ...
    @Override
    public ApiResponse assignStudentToTrip(Integer ownerId, Integer tripId, Map<String, Object> request) {
        System.out.println("🔍 assignStudentToTrip called with ownerId: " + ownerId + ", tripId: " + tripId);
        ...
        try {
            ...
            // Check if student is already assigned to this trip
            boolean alreadyAssigned = tripStudentRepository.existsByTripAndStudent(trip, student);
            if (alreadyAssigned) {
                return new ApiResponse(false, "Student is already assigned to this trip", null);
            }

            // Create trip-student assignment
            TripStudent tripStudent = TripStudent.builder()
                    .trip(trip)
                    .student(student)
                    .isActive(true)
                    .createdBy(owner.getName())
                    .createdDate(LocalDateTime.now())
                    .build();

            TripStudent saved = tripStudentRepository.save(tripStudent);
            System.out.println("🔍 Student assigned to trip successfully with ID: " + saved.getTripStudentId());

            return new ApiResponse(true, "Student assigned to trip successfully", null);
            
        } catch (Exception e) {
            System.out.println("🔍 Error in assignStudentToTrip: " + e.getMessage());
            e.printStackTrace();
            return new ApiResponse(false, "Error assigning student to trip: " + e.getMessage(), null);
        }
    }

    @Override
    public ApiResponse removeStudentFromTrip(Integer ownerId, Integer tripId, Integer studentId) {
        System.out.println("🔍 removeStudentFromTrip called with ownerId: " + ownerId + ", tripId: " + tripId + ", studentId: " + studentId);
        try {
            // Validate vehicle owner exists
            VehicleOwner owner = vehicleOwnerRepository.findById(ownerId)
                    .orElseThrow(() -> new ResourceNotFoundException("Vehicle owner not found with ID: " + ownerId));
            ...
            // Find and remove trip-student assignment
            TripStudent tripStudent = tripStudentRepository.findByTripAndStudent(trip, student)
                    .orElseThrow(() -> new ResourceNotFoundException("Student is not assigned to this trip"));

            tripStudentRepository.delete(tripStudent);
            System.out.println("🔍 Student removed from trip successfully");

            return new ApiResponse(true, "Student removed from trip successfully", null);
            
        } catch (Exception e) {
            System.out.println("🔍 Error in removeStudentFromTrip: " + e.getMessage());
            e.printStackTrace();
            return new ApiResponse(false, "Error removing student from trip: " + e.getMessage(), null);
        }
    }

    @Override
    public ApiResponse getTripStudents(Integer ownerId, Integer tripId) {
        System.out.println("🔍 getTripStudents called with ownerId: " + ownerId + ", tripId: " + tripId);
        try {
            // Validate vehicle owner exists
            VehicleOwner owner = vehicleOwnerRepository.findById(ownerId)
                    .orElseThrow(() -> new ResourceNotFoundException("Vehicle owner not found with ID: " + ownerId));
            ...
            // Get all students assigned to this trip
            List<TripStudent> tripStudents = tripStudentRepository.findByTrip(trip);
            
            List<Map<String, Object>> students = tripStudents.stream()
                    .map(ts -> {
                        Student student = ts.getStudent();
                        Map<String, Object> studentMap = new HashMap<>();
                        studentMap.put("tripStudentId", ts.getTripStudentId());
                        studentMap.put("studentId", student.getStudentId());
                        studentMap.put("firstName", student.getFirstName());
                        studentMap.put("middleName", student.getMiddleName());
                        studentMap.put("lastName", student.getLastName());
                        ...
                        studentMap.put("grade", student.getGrade());
                        studentMap.put("section", student.getSection());
                        studentMap.put("isActive", ts.getIsActive());
                        studentMap.put("assignedDate", ts.getCreatedDate());
                        return studentMap;
                    })
                    .collect(Collectors.toList());
            
            System.out.println("🔍 Found " + students.size() + " students assigned to trip");
            return new ApiResponse(true, "Trip students retrieved successfully", students);
            
        } catch (Exception e) {
            System.out.println("🔍 Error in getTripStudents: " + e.getMessage());
            e.printStackTrace();
